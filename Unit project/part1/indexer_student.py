...
class Index:
    ...
    def search(self, term):
        ...
        if term in self.index.keys():
            ind = self.index[term]
            msgs = [(i, self.msgs[i])for i in ind]
...
class PIndex(Index):
    ...
    def load_poems(self):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        line = open(self.name, 'r').readlines()
        for i in line:
            self.add_msg_and_index(i.rstrip())
    ...
    def get_poem(self, p):
        ...
        while line <end:
            anotherline = self.get_msg(line)
            if anotherline == p2:
                break
            poem.append(anotherline)
            line += 1
